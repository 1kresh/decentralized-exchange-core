...
contract SimswapPool is ISimswapPool, SimswapERC20, NoDelegateCall, ReentrancyGuard {
    ...
    function _update(uint256 _balance0, uint256 _balance1, uint112 _reserve0, uint112 _reserve1) private {
        ...
        if (timeElapsed != 0 && _reserve0 != 0 && _reserve1 != 0) {
            ...
            unchecked {
                price0CumulativeLast += uint256(FixedPoint112.encode(_reserve1).uqdiv(_reserve0)) * timeElapsed;
                price1CumulativeLast += uint256(FixedPoint112.encode(_reserve0).uqdiv(_reserve1)) * timeElapsed;
            }
        }
    ...
    function mint(address recipient) public override nonReentrant returns (uint256 liquidity) {
        ...
        if (feeOn == true)
            kLast = uint256(slot0.reserve0) * slot0.reserve1; // reserve0 and reserve1 are up-to-date
    ...
    /// @inheritdoc ISimswapPoolActions
    /// @dev This low-level function should be called from a contract which performs important safety checks
    function burn(address recipient) public override nonReentrant returns (uint256 amount0, uint256 amount1) {
        uint112 _reserve0 = slot0.reserve0;
        uint112 _reserve1 = slot0.reserve1;
        ...
        if (feeOn == true)
            kLast = uint(slot0.reserve0) * slot0.reserve1; // reserve0 and reserve1 are up-to-date
    ...
    function swap(uint256 amount0Out, uint256 amount1Out, address recipient, bytes calldata data) public override nonReentrant noDelegateCall {
        ...
        { // scope for _token{0,1}, avoids stack too deep errors
            ...
            if (amount1Out != 0)
                _safeTransfer(_token1, recipient, amount1Out); // optimistically transfer tokens
            if (data.length != 0)
                ISimswapCallee(recipient).simswapCall(msg.sender, amount0Out, amount1Out, data);
