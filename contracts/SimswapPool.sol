...
contract SimswapPool is ISimswapPool, NoDelegateCall {
    ...
    function _update(uint256 balance0, uint256 balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'Simswap: OVERFLOW');
    ...
    function mint(address recipient) external override nonReentrant returns (uint256 liquidity) {
        (uint112 _reserve0, uint112 _reserve1,) = slot0; // gas savings
        uint256 balance0 = balance0();
        uint256 balance1 = balance1();
    ...
    function swap(uint256 amount0Out, uint256 amount1Out, address recipient, bytes calldata data) external override nonReentrant noDelegateCall {
        ...
        if (data.length > 0) ISimswapCallee(to).simswapCall(msg.sender, amount0Out, amount1Out, data);
        balance0 = balance0();
        balance1 = balance1();
        }
    ...
    function skim(address recipient) external override lock {
        address _token0 = token0; // gas savings
        address _token1 = token1; // gas savings
        _safeTransfer(_token0, recipient, balance0() - reserve0);
        _safeTransfer(_token1, recipient, balance1() - reserve1);
    }
